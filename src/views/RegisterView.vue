    ...
    <div class="flex md:w-1/2 justify-center py-10 items-center bg-white">
      <form class="bg-white">
        ...
        <div class="flex items-center border border-gray-400 py-2 px-3 rounded-2xl mb-4">
          ...
          <input class="pl-2 outline-none" type="text" id="nickname" name="nickname" v-model="nickname" placeholder="Apelido" />
        </div>
        <div class="flex items-center border border-gray-400 py-2 px-3 rounded-2xl mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-700" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd" />
          </svg>
          <input class="pl-2 outline-none" type="text" id="password" name="password" v-model="password" placeholder="Senha" />
        </div>
  ...
  methods: {
    ...
    createNewUser() {
      ...
      if (!this.confirmpassword) {
        alert('Insira uma confirmação de senha.')
        return
      }

      if (this.password !== this.confirmpassword) {
        alert('As senha e a confirmação de senha precisam ser iguais.')
        return
      }

      axios.post('http://localhost:5000/users/register', newUser).then(alert(`O usuário ${this.name} foi criado com sucesso!`));
      this.name = ''
      this.nickname = ''
      this.password = ''
